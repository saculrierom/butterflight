        ...
        if (axis != FD_YAW) {
            ...
            switch (pidProfile->dterm_filter_style) {
                ...
                case KD_FILTER_SP:
                    ...
                    delta = dtermLowpassApplyFn((filter_t *) &dtermLowpass[axis], (rD - previousRateError[axis]) * iDT );
                    previousRateError[axis] = rD;
                    break;
                case KD_FILTER_NOSP:
                    //filter Kd properly, no sp
                    delta = dtermLowpassApplyFn((filter_t *) &dtermLowpass[axis], (pureRD - previousRateError[axis]) * iDT );
